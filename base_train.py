...
from base_datasets import ImageTextPairDataset
...
if __name__ == "__main__":
    ...
    # scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=3, eta_min=1e-8)
    # scheduler = optim.lr_scheduler.CyclicLR(optimizer, base_lr=0.0005, max_lr=0.1,step_size_up=5,mode="triangular")
    scheduler = optim.lr_scheduler.CyclicLR(
        optimizer,
        base_lr=1e-4,
        max_lr=0.1,
        step_size_up=2000*4,
        step_size_down=2000*6,
        mode='triangular2'
    )
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (batch_img, batch_input_ids, batch_attention_mask) in pbar:
            ...
            loss = (ce(image_to_text, label) + ce(text_to_image, label)) * 0.5 / args.batch_size
            loss.backward()
            ...
            wandb.log({
                "Train Iteration Loss" : loss.item(),
        ...
        for data in testloader:
            ...
            val_loss += ce(image_to_text.detach().cpu(), labels)
        ...
        wandb.log({
            ...
            "Validation Loss" : val_loss / total
            })
